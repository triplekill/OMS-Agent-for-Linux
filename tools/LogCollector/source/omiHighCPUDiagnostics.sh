...
if [ "$1" = "--help" ]; then
    ...
    echo "--cpu-threshold     set cpu threashold, default $cpu_threshold%"
...
if [ "$1" = "--runtime-in-min" ]; then
    minutes_val=$2
    if [ $minutes_val -gt 0 ]; then
        let "count = ( $minutes_val * 60 )/ $rescan_interval_sec"
        shift 2
    else
        echo "runtime value of '$minutes_val' is invalid, should be greater than or equal to 1" >& 2; exit 1;
    fi
fi
# optional arguments
